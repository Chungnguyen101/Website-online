...
namespace DATN2.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (cart != null)
                {
                    ...
                    foreach (var sp in cart.Items)
                    {
                        ...
                        strSanPham += "<td>" + DATN2.Common.Common.FormatNumber(sp.TotalPrice, 0) + "</td>";
                        strSanPham += "</tr>";
                    ...
                    contentCustomer = contentCustomer.Replace("{{ThanhTien}}", DATN2.Common.Common.FormatNumber(thanhtien, 0));
                    contentCustomer = contentCustomer.Replace("{{TongTien}}", DATN2.Common.Common.FormatNumber(TongTien, 0));
                    DATN2.Common.Common.SendMail("ACSHOP", "Đơn hàng #" + order.Code, contentCustomer.ToString(), req.Email);
                    ...
                    contentAdmin = contentAdmin.Replace("{{ThanhTien}}", DATN2.Common.Common.FormatNumber(thanhtien, 0));
                    contentAdmin = contentAdmin.Replace("{{TongTien}}", DATN2.Common.Common.FormatNumber(TongTien, 0));
                    DATN2.Common.Common.SendMail("ACSHOP", "Đơn hàng mới #" + order.Code, contentAdmin.ToString(), ConfigurationManager.AppSettings["EmailAdmin"]);
                    cart.ClearCart();
        ...
        public ActionResult AddToCart(int id, int quantity)
        {
            ...
            var checkProduct = db.Wishlist.FirstOrDefault(x => x.Id == id);
