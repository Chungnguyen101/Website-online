...
jQuery(document).ready(function ($) {
			...
			$('.product-grid').isotope({
				...
				getSortData: {
					price: function (itemElement) {
						var priceEle = $(itemElement).find('.in_product_price').text().trim();
						// Remove any non-numeric characters (like currency symbols)
						var price = parseFloat(priceEle.replace(/[^\d.-]/g, '').replace(',', '.'));
						return price;
					},
					name: '.product_name'
				},
			...
			filterButton.on('click', function () {
				$('.product-grid').isotope({
					filter: function () {
						...
						var priceMin = parseFloat(priceRange.split('-')[0].replace(/[^\d.-]/g, ''));
						var priceMax = parseFloat(priceRange.split('-')[1].replace(/[^\d.-]/g, ''));
						var itemPriceText = $(this).find('.in_product_price').text().trim();
						var itemPrice = parseFloat(itemPriceText.replace(/[^\d,-]/g, '').replace(',', '.'));
	...
	*/
	...
	function initPriceSlider() {
		console.log("initPriceSlider called");
		$("#slider-range").slider({
			range: true,
			min: 0,
			max: 1000000,
			...
			slide: function (event, ui) {
				console.log("Slide event triggered: ", ui.values);
		...
		console.log("Initial values set: ", $("#slider-range").slider("values"));
	}
